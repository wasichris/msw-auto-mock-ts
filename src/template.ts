...
export function overrideHandlerTemplate(op: Operation, options: ConfigOptions) {
  ...
  const reqType = getRequestType(op.requestBody as OpenAPIV3.RequestBodyObject, options.camel);
  const resType = getResType(op.response[0], options.camel);
  const overrideHandlerName = getOverrideHandlerName(op.id, options.camel);
  const code = `/**
* This file is AUTO GENERATED by [msw-auto-mock-ts](https://github.com/wasichris/msw-auto-mock-ts)
...
const ${overrideHandlerName} = async (
  ${hasValueToShow(reqType, `request: ${reqType}`)}
) => {
...
export default ${overrideHandlerName};
`;
